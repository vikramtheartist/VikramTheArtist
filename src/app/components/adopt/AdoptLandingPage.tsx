...
export const AdoptLandingPage: React.FC<AdoptLandingPageProps> = ({
  ...
  return (
    <div className="adopt-page-wrapper selection:bg-indigo-500 selection:text-white relative">
      ...
      {/* ── UNIFIED PINNED SCROLL THEATRE: HERO + THE CORE PROBLEM ─── */}
      <div className="relative w-full" style={{ height: "230vh" }}>
        {/* Sticky Pinned Viewport Stage (Pinned for the entire 230vh scroll journey) */}
        <div className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center">
          
          {/* 1. LAYER 1: HERO LIVING FLUID BACKGROUND (BLURS INTO FLUID BACKDROP ON SCROLL) */}
          <div
            className="absolute inset-0 w-full h-full pointer-events-none select-none z-0 overflow-hidden flex items-center justify-end transition-all duration-75 ease-out"
            style={{
              filter: `blur(${Math.min(scrollY * 0.034, 30)}px)`,
              transform: `scale(${1 + Math.min(scrollY * 0.00015, 0.08)})`,
            }}
          >
          ...
          {/* 2. LAYER 2: HERO INTERACTIVE CONTENT (FADES & BLURS OUT AS USER SCROLLS) */}
          <div
            className="max-w-[1440px] mx-auto px-6 sm:px-10 lg:px-12 w-full absolute inset-0 z-10 flex flex-col justify-between pt-24 pb-10 lg:pt-28 lg:pb-12 will-change-transform pointer-events-auto"
            style={{
              opacity: Math.max(1 - scrollY * 0.0035, 0),
              filter: `blur(${Math.min(scrollY * 0.038, 22)}px)`,
              transform: `translate3d(0, ${-scrollY * 0.18}px, 0) scale(${Math.max(1 - scrollY * 0.0003, 0.92)})`,
              pointerEvents: scrollY > 260 ? "none" : "auto",
              visibility: scrollY > 380 ? "hidden" : "visible",
            }}
          >
          ...
          {/* 3. LAYER 3: "THE CORE PROBLEM" (FULL WIDTH, FLOATING PARALLAX, REPLACES HERO IN CENTER) */}
          <div
            id="problem"
            className="absolute inset-0 z-20 w-full h-full flex items-center justify-center pointer-events-auto will-change-transform"
            style={{
              opacity: Math.min(Math.max((scrollY - 160) * 0.0038, 0), 1),
              filter: `blur(${Math.max(14 - Math.max(scrollY - 160, 0) * 0.05, 0)}px)`,
              transform: `translate3d(0, ${Math.max(80 - Math.max(scrollY - 160, 0) * 0.28, 0)}px, 0)`,
              pointerEvents: scrollY < 180 ? "none" : "auto",
              visibility: scrollY < 120 ? "hidden" : "visible",
            }}
          >
            {/* Floating Parallax Iceberg Visual in Center with Multiply Blend */}
            <div className="absolute inset-0 select-none pointer-events-none z-0 overflow-hidden flex items-center justify-center">
              <div
                ...
                style={{
                  transform: `translate3d(0, ${(scrollY - 450) * -0.045}px, 0) scale(${1 + Math.sin(scrollY * 0.002) * 0.012})`,
                }}
              >
                <img
              ...
              {/* Ambient radial halo to lift crystal luminosity */}
              <div
                className="absolute inset-0 pointer-events-none"
                style={{
                  background:
                    "radial-gradient(ellipse at 50% 50%, rgba(255,255,255,0.45) 0%, rgba(255,255,255,0) 65%)",
                }}
              />
            </div>
            ...
            <div className="max-w-[1440px] mx-auto px-6 sm:px-10 lg:px-14 w-full relative z-10 my-auto flex flex-col justify-between min-h-[640px]">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                {/* Top Right: ABOVE THE SURFACE Callout Card (Floating with Gentle Bobbing) */}
                <div className="lg:col-span-6 flex justify-start lg:justify-end">
                  <div className="relative rounded-[24px] bg-white/92 backdrop-blur-xl border border-white/80 p-5 shadow-[0_15px_35px_-8px_rgba(99,102,241,0.12)] w-full max-w-[320px] text-left lg:mr-4 mt-2 lg:mt-2 animate-float-gentle">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-end pt-8 sm:pt-12">
                ...
                {/* Bottom Right: BELOW THE SURFACE Callout Card (Floating with Reverse Bobbing) */}
                <div className="lg:col-span-6 flex justify-start lg:justify-end">
                  <div className="relative rounded-[24px] bg-white/92 backdrop-blur-xl border border-white/80 p-5 shadow-[0_15px_35px_-8px_rgba(99,102,241,0.12)] w-full max-w-[320px] text-left lg:mr-4 mb-2 animate-float-reverse">
